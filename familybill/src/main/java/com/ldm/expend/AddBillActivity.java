...
public class AddBillActivity extends Activity implements OnClickListener {
	...
	private void saveExpendsData() {
		saveData = new String[] { new SimpleDateFormat("yyyy-MM-dd").format(new Date()), mFoodEdt.getText().toString().trim(), mArticlesEdt.getText().toString().trim(), mRemarkEdt.getText().toString().trim() };
		if (canSave(saveData)) {
	...
	private void saveIncomeData() {
		saveData = new String[] { new SimpleDateFormat("yyyy-MM-dd").format(new Date()),
				mSalaryEdt.getText().toString().trim(), mPrizeEdt.getText().toString().trim(),  mRemarkEdt.getText().toString().trim() };
		if (canSave(saveData)) {
			ContentValues values = new ContentValues();
